...
class Reporting extends CI_Controller {
	...
	public function index()
	{
		...
		$data['query'] = $this->m_reporting->selectSummaryTransaksi($this->session->userdata('id_user'));
		$this->load->view('adminmobitick/v_dashboard', $data);
	}

	
	public function todayTransaction($id)
	{
		$this->telah_login();
		$data['base_url'] = $this->base_url;
		$data['images'] = '/new/'.$this->images;
		$data['css'] = '/new/'.$this->css;
		$data['js'] = '/new/'.$this->js;
		$data['plugins'] = '/new/'.$this->plugins;
		//$data['query'] = $this->m_reporting->selectAllTransaksi($this->session->userdata('id_user'));
		$data['query'] = $this->m_reporting->reportTransaksiBusToday($this->session->userdata('id_user'),$id);
		$this->load->view('adminmobitick/v_dashboard', $data);
	}	
	...
	public function allTransaction($id)
	{
		...
		$data['plugins'] = '/new/'.$this->plugins;
		//$data['query'] = $this->m_reporting->selectAllTransaksi($this->session->userdata('id_user'));
		$data['query'] = $this->m_reporting->reportTransaksiBusOverall($this->session->userdata('id_user'),$id);
		$this->load->view('adminmobitick/v_dashboard', $data);
	}
